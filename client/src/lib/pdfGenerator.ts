...
export class SlidePDFGenerator {
  ...
  private setSubtitle(text: string, y: number) {
    ...
    // Wrap subtitle text to fit within content width
    const lines = this.doc.splitTextToSize(text, CONTENT_WIDTH);
    this.doc.text(lines, MARGIN, y);
    this.doc.setTextColor(0, 0, 0);
  ...
  private async generateTitlePage() {
    ...
    // Add ProblemOps logo at the top
    try {
      const logoImg = new Image();
      logoImg.src = '/problemops-logo.png';
      await new Promise((resolve, reject) => {
        logoImg.onload = resolve;
        logoImg.onerror = reject;
      });
      // Logo dimensions - maintain aspect ratio (original is wide)
      const logoWidth = 80;
      const logoHeight = logoWidth * (logoImg.height / logoImg.width);
      this.doc.addImage(logoImg, 'PNG', (PAGE_WIDTH - logoWidth) / 2, 30, logoWidth, logoHeight);
    } catch (e) {
      // If logo fails to load, continue without it
      console.warn('Could not load logo for PDF:', e);
    }
    
    // Title: [Company name] Cross-Functional Team Readiness Assessment
    const companyName = this.data.companyInfo.name || 'Your Organization';
    const titleLine1 = companyName;
    const titleLine2 = 'Cross-Functional Team Readiness Assessment';
    
    this.doc.setFontSize(28);
    this.doc.setFont('helvetica', 'bold');
    this.doc.setTextColor(PRIMARY_COLOR[0], PRIMARY_COLOR[1], PRIMARY_COLOR[2]);
    this.doc.text(titleLine1, PAGE_WIDTH / 2, 85, { align: 'center' });
    
    this.doc.setFontSize(20);
    this.doc.setFont('helvetica', 'normal');
    this.doc.setTextColor(60, 60, 60);
    this.doc.text(titleLine2, PAGE_WIDTH / 2, 100, { align: 'center' });
    
    if (this.data.companyInfo.team) {
      this.doc.setFontSize(16);
      this.doc.setFont('helvetica', 'italic');
      this.doc.setTextColor(100, 100, 100);
      this.doc.text(this.data.companyInfo.team, PAGE_WIDTH / 2, 115, { align: 'center' });
    }
    ...
    this.doc.text('Report generated by ProblemOps Institute', PAGE_WIDTH / 2, 145, { align: 'center' });
    ...
    this.doc.text(today, PAGE_WIDTH / 2, 160, { align: 'center' });
    
    // Website
    this.doc.setTextColor(PRIMARY_COLOR[0], PRIMARY_COLOR[1], PRIMARY_COLOR[2]);
    this.doc.text('https://problemops.com', PAGE_WIDTH / 2, 175, { align: 'center' });
    
    // Founder info
    this.doc.setTextColor(60, 60, 60);
    this.doc.setFontSize(12);
    this.doc.text('Morgan Denner, Founder and Head Trainer', PAGE_WIDTH / 2, 210, { align: 'center' });
    this.doc.text('mdenner@problemops.com', PAGE_WIDTH / 2, 222, { align: 'center' });
  ...
  public async download(filename: string = 'team-readiness-assessment.pdf') {
    await this.generate();
